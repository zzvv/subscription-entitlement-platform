...
func TestRepeatedConfirmationReturnsPersistedSubscription(t *testing.T) {
	store := repository.NewStore()
	service := NewReceiptService(store)
	first, err := service.Confirm(context.Background(), domain.NewCommand("sub-a", "tenant-a", "standard", "activate"))
	if err != nil {
		t.Fatalf("first confirmation: %v", err)
	}
	second, err := service.Confirm(context.Background(), domain.NewCommand("sub-b", "tenant-a", "standard", "activate"))
	if err != nil {
		t.Fatalf("repeated confirmation: %v", err)
	}
	stored, ok := store.Find(context.Background(), "tenant-a/standard")
	if !ok {
		t.Fatal("missing persisted subscription")
	}
	if second != stored || second != first {
		t.Fatalf("repeated confirmation returned an entity that was not persisted: first=%+v second=%+v stored=%+v", first, second, stored)
	}
}
