...
func TestSaveDoesNotWriteAfterCancellationDuringLockWait(t *testing.T) {
	store := NewStore()
	ctx, cancel := context.WithCancel(context.Background())
	store.SetSaveBeforeLockForTest(cancel)

	err := store.Save(ctx, "tenant-a/standard", domain.NewEntity("sub-a", "tenant-a", "standard"))
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("expected cancellation before state write, got %v", err)
	}
	if _, ok := store.Find(context.Background(), "tenant-a/standard"); ok {
		t.Fatal("cancelled save wrote subscription state")
	}
}
