...
func (s *Store) CommitWithReceipt(ctx context.Context, key string, value domain.Entity, receipt domain.Receipt) error {
	...
	// Verify the notification receipt can land before exposing the subscription
	// state, otherwise a failed receipt commit would leave a confirmed
	// subscription with no matching notification record. The injected fault is
	// one-shot: once it has surfaced on a commit, it is cleared so a subsequent
	// normal confirmation can land both sides together again rather than being
	// stuck failing forever.
	if err := s.receiptCommitErr; err != nil {
